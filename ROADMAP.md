...
- scheduled/system operations with truthful `user.presence = not_present` attribution;
...
Guidon can protect a practical Windows file/directory scope repeatedly, perform controlled recovery/deletion operations under the defined authorization model, and explain exactly who/what performed each meaningful action.
...
Guidon can rebuild a supported Windows system from recovery data and regain controlled administrative access without depending on the failed environment's AD trust or an unknown historic local password.
...
Prove that Guidon's own control plane and mutable catalog are not hidden single points of failure.
...
- recovery when repository datasets are moved/imported to a new host; and
- documented appliance/storage replacement procedure.

## Exit gate

Starting from intact authoritative Guidon recovery data but without the original Controller/catalog, a clean Guidon installation can rediscover supported recovery points, verify them, rebuild operational indexes, and recover a supported workload.
...
- independent security/code review; and
- pilot deployment/rollback procedure.

## Pilot exit gate

Guidon can be installed, operated, upgraded, intentionally broken, recovered, audited, and removed in a controlled pilot without relying on undocumented developer knowledge.
